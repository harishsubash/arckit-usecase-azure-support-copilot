...
| NFR-002 | System availability ≥ 99.5% measured monthly for the orchestration path; on outage, falls back to human-only queue (see ARC-001-PLAT) | Must |
...
Requirements in this document are the basis for the Architecture Decision Records (`decisions/ARC-001-ADR-*`) and the Platform Design (`ARC-001-PLAT-v1.0.md`). FR-001–FR-005 and NFR-005 are the primary drivers for ADR-001 and ADR-002.
